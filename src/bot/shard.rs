...
pub async fn process_event(ctx: Context, event: Event) {
    match event {
        Event::Ready(info) => {
            ...
            let original_app_id = ctx.app().application_id().await;
            ...
            if new_app_id != original_app_id {
                ...
                ctx.app().override_application_id(new_app_id).await;
            }
        }
        _ => {}
    }
...
pub async fn main(app: App, mut shard: Shard) {
    let context = Context::new(&app, shard.id());
    let tasks = TaskTracker::new();

    loop {
        let action = next_event(&app, &mut shard).await;
...
async fn next_event(app: &App, shard: &mut Shard) -> ShardAction {
